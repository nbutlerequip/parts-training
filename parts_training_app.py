...
def save_to_google_sheets():
    ...
    5. Add credentials to Streamlit secrets or as environment variable
    ...
    try:
        ...
        if 'gcp_service_account' in st.secrets:
            ...
            SHEET_ID = st.secrets.get("TRAINING_SHEET_ID", "YOUR_SHEET_ID_HERE")
            sheet = gc.open_by_key(SHEET_ID)
            worksheet = sheet.worksheet("Results")
            
            # Prepare data for each completed training
        ...
        # Results are still saved in session state
        return False
