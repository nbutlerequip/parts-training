...
def show_training():
    ...
    if not st.session_state.show_feedback:
        ...
        for choice in choices:
            ...
            bg_color, border_color = feedback_colors.get(choice_score, ('#f8f9fa', '#6b7280'))
            score_label = score_labels.get(choice_score, choice_score)
            
            # Mark which one they selected
            yours_text = " (YOUR CHOICE)" if choice == selected else ""
            
            # Use Streamlit's native container with custom styling
            with st.container():
                st.markdown(
                    f"""<div style="background: {bg_color}; border: 2px solid {border_color}; border-radius: 8px; padding: 16px; margin-bottom: 12px;">
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                        <span style="font-weight: 700; font-size: 15px;">{choice.upper()}</span>
                        <span style="background: {border_color}; color: white; font-size: 11px; padding: 4px 10px; border-radius: 20px; font-weight: 600; text-transform: uppercase;">{score_label}</span>
                        {"<span style='background: #333; color: white; font-size: 11px; padding: 4px 10px; border-radius: 20px; font-weight: 600;'>YOUR CHOICE</span>" if choice == selected else ""}
                    </div>
                    <div style="font-style: italic; color: #4b5563; margin-bottom: 10px; font-size: 14px;">"{choice_text}"</div>
                    <div style="font-size: 14px; color: #374151; line-height: 1.6;">{choice_explanation}</div>
                    </div>""",
                    unsafe_allow_html=True
                )
        ...
        # Key insight
        st.markdown(
            f"""<div style="background: #f0f9ff; border: 1px solid #0ea5e9; border-radius: 8px; padding: 14px 18px; margin-top: 20px;">
            <div style="font-weight: 600; color: #0369a1; font-size: 13px; margin-bottom: 4px;">Key Insight</div>
            <div style="font-size: 14px; color: #1e40af; line-height: 1.5;">{scenario['key_insight']}</div>
            </div>""",
            unsafe_allow_html=True
        )
