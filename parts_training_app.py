...
def show_training():
    ...
    if not st.session_state.show_feedback:
        ...
        # Build all feedback HTML in one string
        feedback_html = "<div style='font-weight: 600; margin-bottom: 16px; color: #374151;'>Results</div>"
        ...
        for choice in choices:
            ...
            feedback_class = feedback_classes.get(choice_score, 'feedback-good')
            badge_class = badge_classes.get(choice_score, 'badge-good')
            score_label = score_labels.get(choice_score, choice_score)
            
            # Mark which one they selected
            yours_badge = '<span class="feedback-badge badge-yours">Your Choice</span>' if choice == selected else ''
            
            feedback_html += f'''
            <div class="feedback-box {feedback_class}">
                <div class="feedback-header">
                    <span class="feedback-letter">{choice.upper()}</span>
                    <span class="feedback-badge {badge_class}">{score_label}</span>
                    {yours_badge}
                </div>
                <div class="feedback-quote">"{choice_text}"</div>
                <div class="feedback-explanation">{choice_explanation}</div>
            </div>
            '''
        ...
        <div class="key-insight">
            ...
            <div class="key-insight-text">{scenario['key_insight']}</div>
        </div>
        '''
